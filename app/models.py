...
Protocol = Literal["WIFI", "BLE", "BT", "LORA", "CELLULAR"]
...
class MeasurementIn(BaseModel):
    ...
    reliability_requirement: float | None = Field(default=None, ge=0, le=1)
    ...
    wifi_channel: int | None = Field(default=None, ge=1, le=196)
    ble_phy: str | None = Field(default=None, max_length=50)
    bt_mode: str | None = Field(default=None, max_length=50)
    lora_sf: int | None = Field(default=None, ge=5, le=12)
    lora_bw: int | None = Field(default=None, gt=0)
    lora_cr: str | None = Field(default=None, max_length=50)
    cellular_generation: str | None = Field(default=None, max_length=20)
    ...
    def normalise_protocol(cls, value: str) -> str:
        aliases = {"WI-FI": "WIFI", "BLUETOOTH": "BT", "BLUETOOTH_CLASSIC": "BT", "4G": "CELLULAR", "5G": "CELLULAR"}
        return aliases.get(str(value).upper(), str(value).upper())

    @model_validator(mode="after")
    def validate_packet_counts(self) -> "MeasurementIn":
        if self.packets_sent is not None and self.packets_received is not None:
